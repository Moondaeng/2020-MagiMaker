...
namespace Network
{
    public static class CPacketFactory
    {
        ...
        /*****************************추가 부분***********************************/
        public static CPacket CreateMonsterIdlePacket(int monsterId)
        {
            byte messageSize = 4;

            CPacket packet = new CPacket((int)messageSize);

            packet.WriteHeader(messageSize, (int)EInGameMonster.MonsterIdle);
            packet.Write(monsterId);

            return packet;
        }

        public static CPacket CreateMonsterTracePacket(int monsterId, int playerId)
        {
            byte messageSize = 8;

            CPacket packet = new CPacket((int)messageSize);

            packet.WriteHeader(messageSize, (int)EInGameMonster.MonsterTrace);
            packet.Write(monsterId).Write(playerId);

            return packet;
        }

        public static CPacket CreateMonsterAttackPacket(int monsterId, int playerId)
        {
            byte messageSize = 8;

            CPacket packet = new CPacket((int)messageSize);

            packet.WriteHeader(messageSize, (int)EInGameMonster.MonsterAttack);
            packet.Write(monsterId).Write(playerId);

            return packet;
        }

        public static CPacket CreateMonsterSkillPacket(int monsterId, int skillNum)
        {
            byte messageSize = 8;

            CPacket packet = new CPacket((int)messageSize);

            packet.WriteHeader(messageSize, (int)EInGameMonster.MonsterSkill);
            packet.Write(monsterId).Write(skillNum);

            return packet;
        }

        public static CPacket CreateMonsterSkillPacket(int monsterId, int skillNum, int playerId)
        {
            byte messageSize = 12;
            ...
            packet.WriteHeader(messageSize, (int)EInGameMonster.MonsterSkill);
            packet.Write(monsterId).Write(skillNum).Write(playerId);

            return packet;
        }
        public static CPacket CreateOrderMonsterAction(int monsterId, int skillNum)
        {
            byte messageSize = 8;
