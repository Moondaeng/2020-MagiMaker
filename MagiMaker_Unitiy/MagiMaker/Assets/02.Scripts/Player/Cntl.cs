    ...
    void CheckClick()
    {
        if (Input.GetMouseButton(0))
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                bool stringExists = s.Contains("Room") || s.Contains("Portal");
                // 누른게 지형이면, 이동함.
                if (stringExists)
                {                
                    // PlayerFSM에 정의한 
            ...
            else if (hit.collider.gameObject.tag == "Monster")//마우스 클릭한 대상이 적 캐릭터인 경우
            {
                player.GetComponent<PlayerFSM>().AttackMonster(hit.collider.gameObject);
            }
