...
public class CStoreController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < _slotRoot.childCount - 2; i++)
        {
            ...
            slot.SetItem(CItemManager.instance.DropRandomItem(CCreateMap.instance.StageNumber, CConstants.EQUIP_ITEM_TYPE));
        ...
        {
            var slot = _slotRoot.GetChild(i).GetComponent<CSlotController>();
            slot.SetItem(CItemManager.instance.DropRandomItem(CCreateMap.instance.StageNumber, CConstants.CONSUM_ITEM_TYPE));
