...
public class CController : MonoBehaviour
{
    ...
    private void UseSkill(int number)
    {
        ...
        else
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                bool? isSkillUse = player.GetComponent<CPlayerSkill>().UseSkillToPosition(number, hit.point);
                // 성공 시 플레이어 스킬 사용 이벤트 수행
                if(isSkillUse == true)
                {
                    player.GetComponent<CCntl>().SkillAction(2);
    ...
    private void CallCombo(int skillNumber)
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            bool? isSkillUse =  player.GetComponent<CPlayerSkill>().UseComboSkillToPosition(skillNumber, hit.point);
            if(isSkillUse == true)
            {
