...
public class CGoblinFSM : MonoBehaviour
{
    public enum State
    {
        Idle,  //정지
    ...
    float chaseDistance = 10f; // 플레이어를 향해 몬스터가 추적을 시작할 거리
    ...
    void Start()
    {
        ...
        // 스켈레톤의 스텟을 받아옴
        myPara = GetComponent<CGoblinPara>();
        myPara.deadEvent.AddListener(CallDeadEvent);

        // 첫 상태를 Idle
        ChangeState(State.Idle, CGoblinAni.IDLE);
    ...
    {
        string selection = "GoblinAni." + motion + 1;
        ChangeState(State.Attack, selection);
    }
    ...
    void IdleState()
    {
        ...
        {
            ChangeState(State.Chase, CGoblinAni.WALK);
        }
    }
    ...
    void AttackState()
    {
        ...
        {
            attackTimer = 0f;
            ChangeState(State.Chase, CGoblinAni.RUN);
        }
        else
        {
